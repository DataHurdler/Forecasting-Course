...
ROOT_ALLOW = {
    ...
    # the published course URL inside them. The SYLLABUS is no longer among them --
    # it moved to syllabi/ on 2026-09-01, one file per term, kept forever.
...
ROOT_ALLOW_DIRS = {
    ...
    # book/ holds the Quarto book project — _quarto.yml, the preface and the
    # theme. Its chapters/ subdirectory is generated by scripts/build_book.py and
    # is gitignored; only the project skeleton is tracked. workbook/ is the same
    # arrangement for the labs and assignments, generated by build_workbook.py.
    "book", "workbook",
    # syllabi/ holds one syllabus per term, kept forever and never edited after its
    # term starts (2026-09-01). It is deliberately an ARCHIVE that grows: the course
    # runs in person in fall and online in spring, so each term's document differs,
    # and the record of what students were told is worth more than a tidy root.
    # See syllabi/README.md.
    "syllabi",
}
