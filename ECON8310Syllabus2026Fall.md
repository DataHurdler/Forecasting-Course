...
**Naming and placement.** One folder per submission, named for the assignment exactly as it appears in the schedule — `hw01_part1`, `hw05_part2`, and so on. Run `python scripts/check_my_submission.py` in the environment repository before you push — it takes no arguments and tells you what is missing.

**Submitting.** Three ways, and any of them is fine: the command line, [GitHub Desktop](https://desktop.github.com), or your browser — on github.com, open `submissions/`, choose **Add file → Upload files**, and drag your folder in. The last needs no git knowledge at all.

Commit your finished work with a clear message. You do not need one commit per prompt.
