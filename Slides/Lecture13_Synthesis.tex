...
\begin{frame}{Lesson 1: The Benchmark Is a Real Competitor}
  ...
  model} you fitted --- and the reason is countable. Holt-Winters must estimate \textbf{52
  seasonal indices from 4.3 annual cycles} of training data: about four observations per
  parameter. Seasonal naive estimates \textbf{nothing}, and the level it carries moves
  $+3.7\%$, $+1.3\%$, $-4.4\%$, $+2.9\%$ year over year --- stable inside $\pm 5\%$.
...
\begin{frame}{Lesson 2: More Sophisticated Did Not Mean Better}
  ...
  \begin{center}
    ...
    \begin{tabular}{p{3.6cm}p{8.7cm}}
      ...
      \textbf{Transformer lost to an RNN} & 67{,}329 parameters against roughly \textbf{230
      effectively independent} 26-week blocks --- ours overlap 96\%. \\[3pt]
