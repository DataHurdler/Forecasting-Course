...
\begin{frame}{The Scoreboard}
  ...
  \begin{center}
    ...
    \begin{tabular}{lrl}
      ...
      Transformer encoder & 895 & Lecture 9 Part 2 \\
      Random forest & 899 & Homework 3 \\
      LSTM & 987 & Lecture 9 Part 1 \\
      1D CNN, best variant & 1{,}202 & Lecture 8 \\
      \midrule
  ...
  is at the top; the architecture that reshaped natural language processing is in the middle;
  the two most-hyped deep learning models are near the bottom.
\end{frame}
...
\begin{frame}{Lesson 2: More Sophisticated Did Not Mean Better}
  ...
  \begin{center}
    ...
    \begin{tabular}{p{3.6cm}p{8.7cm}}
      \toprule
      \textbf{LSTM lost to a plain RNN} & At three window lengths. Gating fixes long-range
      forgetting; a 26-week window does not suffer from it. Four times the parameters, no gain. \\[3pt]
...
% --- Slide: silent errors ----------------------------------------------------------
\begin{frame}{The Errors That Fail Silently}
  ...
  \begin{center}
    ...
    \begin{tabular}{p{3.5cm}p{8.8cm}}
      ...
      real feature --- purely for having many distinct values. \\[2pt]
      Ignoring $\hat{R}$ and divergences & Our first two Bayesian fits returned reasonable-looking
      forecasts while failing convergence. \\
      \bottomrule
    \end{tabular}
    }
  \end{center}

  \textbf{The common thread:} the failure mode is not a crash, it is a number that looks fine ---
  which is why the discipline is procedural. Benchmark first, split by time, check the
  diagnostic, plant a control.
\end{frame}
