...
In Homework 1 the seasonal-naive benchmark **beat every exponential smoothing and ARIMA model** you fitted. That was not a trick question — it is what the data supports at that horizon.
...
Four times this course, the more advanced method lost — and each time for a reason we could name.
...
| **Transformer lost to an RNN** | With 15$\times$ the parameters. Its advantages are advantages *at scale*, and 5,940 windows is not scale. |
| **CNNs lost to LASSO** | A seven-week receptive field cannot see an annual lag. |
| **Tuning made XGBoost worse** | Nine fits, and the defaults were three RMSE better. |
