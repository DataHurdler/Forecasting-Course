...
**The ordering is not the one anyone predicts in Week 1.** A 46-coefficient linear model is at the top. The two architectures with the most attention behind them — the LSTM and the Transformer — finished fifth and sixth of eight, behind a plain RNN with a fraction of their parameters.
...
| | |
|---|---|
| **LSTM lost to a plain RNN** | At three window lengths. Gating fixes long-range forgetting; a 26-week window does not suffer from it. Four times the parameters, no gain. |
