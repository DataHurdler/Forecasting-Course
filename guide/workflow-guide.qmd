...
```bash
./scripts/backtest.sh          # all twelve gates green on a clean tree
...
|---|---|---|---|---|---|
| Backtest (12 gates) | scripts, pre-commit + CI | repo-wide consistency/currency, **plus the gate layer itself** — ledger coverage and hook wiring (gate 11), the guard hooks re-fired against their target failures (gate 12) | **closed** — commit blocked | `--no-verify` | gate output, CI log |
...
```bash
./scripts/backtest.sh    # twelve gates: consistency, skill integrity, currency, links,
                         # site-index completeness, book-preface claims, spec, staleness,
