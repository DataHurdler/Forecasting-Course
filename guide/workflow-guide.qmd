...
```bash
./scripts/backtest.sh          # all eleven gates green on a clean tree
...
|---|---|---|---|---|---|
| Backtest (11 gates) | scripts, pre-commit + CI | repo-wide consistency/currency, **plus the gate layer itself** — ledger coverage and hook wiring (gate 10), the guard hooks re-fired against their target failures (gate 11) | **closed** — commit blocked | `--no-verify` | gate output, CI log |
...
```bash
./scripts/backtest.sh    # eleven gates: consistency, skill integrity, currency, links,
                         # site-index completeness, spec, staleness, hygiene, derived counts,
                         # ledger coverage, hook battery
```
