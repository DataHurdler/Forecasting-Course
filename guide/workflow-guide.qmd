...
```bash
./scripts/backtest.sh          # all thirteen gates green on a clean tree
...
|---|---|---|---|---|---|
| Backtest (13 gates) | scripts, pre-commit + CI | repo-wide consistency/currency, **plus the gate layer itself** — ledger coverage and hook wiring (gate 12), the guard hooks re-fired against their target failures (gate 13) | **closed** — commit blocked | `--no-verify` | gate output, CI log |
...
```bash
./scripts/backtest.sh    # thirteen gates: consistency, skill integrity, currency, links,
                         # site-index completeness, book-preface claims, assignment sync, spec,
                         # staleness, hygiene, derived counts, ledger coverage, hook battery
```
